...
void mark(int x, int y, unsigned char *image, int width, int height, int depth) {
	int x1, y1;
	int off, layer;

	for (x1 = x - 2; x1 <= x + 2; x1++) {
		for (y1 = y - 2; y1 <= y + 2; y1++) {
			off = (int)x1 * depth + (int)y1 * width * depth;
			for (layer = 0; layer < depth; layer++) {
				image[off + layer] = 0xFF;
			}
		}
	}
}
...
void process(unsigned char *image, int width, int height, int depth, char *filename,
	...
	mark(x1, y1, image, width, height, depth);
	mark(x2, y2, image, width, height, depth);
	mark(x3, y3, image, width, height, depth);

	output(image, width, height, depth, filename, 0, 0, width, height);
}
